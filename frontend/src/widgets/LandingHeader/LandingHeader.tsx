  ...
  return (
    <header className="flex justify-between items-center w-svw py-4 md:py-6 z-10 absolute left-0 px-4 sm:px-6 md:px-10 lg:px-14">
      ...
      <div className="hidden md:flex gap-3">
        ...
        >
          {theme === "light" ? (
            <Sun className="text-font-primary w-5 h-5 lg:w-6 lg:h-6" />
          ) : (
            <Moon className="text-font-primary w-5 h-5 lg:w-6 lg:h-6" />
          )}
        </button>
        <Link
          to="/login"
      ...
      <div className="flex md:hidden gap-2">
        <button
          className="bg-theme-button rounded-full p-2.5 cursor-pointer shadow-[0_2px_4px_#00000025]"
          onClick={toggleTheme}
        >
          {theme === "light" ? (
            <Sun className="text-font-primary w-5 h-5" />
          ) : (
      ...
      {menuOpen && (
        <div className="fixed inset-0 top-14 bg-landing-bg-1/98 backdrop-blur-sm z-50 md:hidden animate-fade-in">
          <nav className="flex flex-col items-center gap-6 pt-10 font-bold text-font-primary text-xl">
            <a
              href="#posibilities"
              className="hover:text-font-primary-hover"
